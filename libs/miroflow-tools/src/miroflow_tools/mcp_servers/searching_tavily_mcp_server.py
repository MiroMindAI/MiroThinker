...
"""
Orchestrator wrapper for the Tavily MCP search server.
Mirrors the structure of searching_google_mcp_server.py, forwarding calls
via stdio MCP to tavily_mcp_server.
"""
...
def filter_tavily_search_result(result_content: str) -> str:
    """Filter Tavily search result content.

    Args:
        result_content: The JSON string result from tavily search
...
async def tavily_search(
    ...
    It is able to retrieve organic search results with titles, URLs, content snippets, and relevance scores.
        ...
        try:
            async with stdio_client(server_params) as (read, write):
                async with ClientSession(
                    ...
                    filtered_result = filter_tavily_search_result(result_content)
